...
<section class="content-header">
  ...
  <ol class="breadcrumb">
    ...
    <li><a href="{{ route( 'booking-hotel-dn.index') }}">
    @if($type == 2)
    Đặt khách sạn Đà Nẵng
    @endif</a></li>
...
<section class="content">
  <div class="row">
    <div class="col-md-12">
      ...
      @endif
      <a href="{{ route('booking-hotel-dn.create') }}" class="btn btn-info btn-sm" style="margin-bottom:5px">Tạo mới</a>
      <div class="panel panel-default">
        <div class="panel-heading">
          <h3 class="panel-title">Bộ lọc</h3>
        </div>
        <div class="panel-body">
          <form class="form-inline" role="form" method="GET" action="{{ route('booking-hotel-dn.index') }}" id="searchForm">
            ...
            <div class="form-group chon-ngay">              
              <input type="text" class="form-control datepicker" autocomplete="off" name="checkin_from" placeholder="@if($time_type == 2) Từ ngày @else Ngày @endif " value="{{ $arrSearch['checkin_from'] }}" style="width: 90px">
            </div>
           
            @if($time_type == 2)
            <div class="form-group chon-ngay den-ngay">              
              <input type="text" class="form-control datepicker" autocomplete="off" name="checkin_to" placeholder="Đến ngày" value="{{ $arrSearch['checkin_to'] }}" style="width: 90px">
            </div>
             @endif
            @endif           
            @if(Auth::user()->role == 1)
            <div class="form-group">
              <input type="text" class="form-control datepicker" autocomplete="off" name="checkout_from" value="{{ $arrSearch['checkout_from'] }}" style="width: 100px" placeholder="Check out">
            </div>
            <div class="form-group">
              <input type="text" class="form-control datepicker" autocomplete="off" name="checkout_to" value="{{ $arrSearch['checkout_to'] }}" style="width: 100px" placeholder="Đến ngày">
            </div>
            @endif            
            <div class="form-group">
            ...
            <div class="form-group">              
              <input type="text" class="form-control" name="email" value="{{ $arrSearch['email'] }}" placeholder="Email">
            </div>                       
            ...
            @if( $items->count() > 0 )
              ...
              <tr id="row-{{ $item->id }}" 
                ...
                <td>     
                  ...
                  <a style="font-size:17px" href="{{ route( 'booking-hotel-dn.edit', $arrEdit) }}">{{ $item->name }}</a>
                  <br>
                  ĐT: {{ $item->phone }}
                  <br>
                  {{ $item->email }}<br>
                ...
                <td class="text-right">
                  ...
                  <input type="radio" name="nguoi_thu_coc{{ $item->id }}" class="change-value" value="1" id="nguoi_thu_coc{{ $item->id }}_1" data-col="nguoi_thu_coc" data-value="1" data-id="{{ $item->id }}" {{ $item->nguoi_thu_coc == 1 ? "checked=checked" : "" }}> <label for="nguoi_thu_coc{{ $item->id }}_1">Sale thu cọc</label><br>
                  <input type="radio" name="nguoi_thu_coc{{ $item->id }}" class="change-value" value="2" id="nguoi_thu_coc{{ $item->id }}_2" data-col="nguoi_thu_coc" data-value="2" data-id="{{ $item->id }}" {{ $item->nguoi_thu_coc == 2 ? "checked" : "" }}> <label for="nguoi_thu_coc{{ $item->id }}_2">KS thu cọc</label>
                ...
                <a href="{{ route( 'booking-payment.index', ['booking_id' => $item->id] ) }}" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-usd"></span></a>         
                  @php $arrEdit = array_merge(['id' => $item->id], $arrSearch) @endphp            
                  <a href="{{ route( 'booking-hotel-dn.edit', $arrEdit ) }}" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>                     
                  @if(Auth::user()->role == 1 && $item->status == 1)
                  <a onclick="return callDelete('{{ $item->title }}','{{ route( 'booking-hotel-dn.destroy', [ 'id' => $item->id ]) }}');" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
                  @endif
