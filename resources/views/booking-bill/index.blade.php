...
<section class="content-header">
  <h1>
    Bill mua sắm/ăn uống
  </h1>
  <ol class="breadcrumb">
    <li><a href="#"><i class="fa fa-dashboard"></i> Dashboard</a></li>
    <li><a href="{{ route( 'booking.index', ['type' => $detailBooking->type] ) }}">
...
<section class="content">
          ...
          <table class="table table-bordered table-hover" id="table-list-data">
            ...
            @if( $items->count() > 0 )
              ...
              <tr id="row-{{ $item->id }}">
                ...
                @if($item->type == 1)                            
                  <a href="{{ route( 'booking-bill.edit', [ 'id' => $item->id ]) }}" class="btn btn-warning btn-sm"><span class="glyphicon glyphicon-pencil"></span></a>                 
                  
                  <a onclick="return callDelete('{{ $item->title }}','{{ route( 'booking-bill.destroy', [ 'id' => $item->id ]) }}');" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash"></span></a>
                  @endif
                </td>
              </tr> 
              @endforeach
  ...
  <form role="form" method="POST" action="{{ route('booking-bill.store') }}" id="dataForm">
    <div class="row">
       <div class="col-md-12">
